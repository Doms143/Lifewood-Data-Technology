              ...
              {useCompactNav ? (
                ...
                <nav className="w-full" ref={desktopNavRef}>
                  <ul className="flex items-center justify-end gap-3 lg:gap-4 whitespace-nowrap">
                    {desktopItems.map((item) => (
                      <li key={`${item.label}-${item.value || item.key || ''}`} className="relative">
                        {item.type === 'submenu' ? (
                          <>
                            <button
                              type="button"
                              onClick={() => setOpenDesktopMenu((prev) => (prev === item.key ? null : item.key))}
                          ...
                          <button
                            ...
                            className={`focus-brand text-[11px] sm:text-xs lg:text-sm font-semibold transition-colors ${
                              item.label === 'Sign In'
                                ? 'ml-1 rounded-full border border-castleton/25 px-3 py-1.5 bg-white/70 hover:bg-castleton hover:text-white'
                                : isItemSelected(item)
                                  ? 'text-[#D79A44]'
                    ...
                    <li key={item.label}>
                      <button
                        ...
                        className={`focus-brand flex items-center gap-2 text-left text-sm sm:text-base leading-tight font-medium transition-colors ${
                          item.label === 'Sign In'
                            ? 'inline-flex rounded-full border border-castleton/25 px-4 py-2 bg-white/70 hover:bg-castleton hover:text-white'
                            : isItemSelected(item)
                              ? 'text-[#D79A44]'
                              : 'text-serpent hover:text-castleton'
                        }`}
